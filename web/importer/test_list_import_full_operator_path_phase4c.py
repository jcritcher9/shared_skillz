"""Phase 4C — Django FE multi-Account review + Django→API boundary.

Network-free only. Proves:
- workflow page renders multi-Account choices (candidate / none / quarantine)
- provision-off Lead-fallback label and provision-on optional-create label
...
- **Django→API boundary** (real API process, no mocked dispatch):
  - provision-disabled: none-of-these force_lead accepted on same run_id
  - provision-configured (ceiling): none-of-these continue_net_new accepted;
    revision advances; empty write grants

No live CRM. FE-CM-4 remains product-default.

Authority:
  mappings_2/codex_context/cross_agent_eval/project_implementations/
    list_import_full_operator_path.md (Phase 4C)
"""
...
FORCE_LEAD_KIND = "force_lead_without_account"
CONTINUE_NET_NEW_KIND = "continue_net_new_account"
FORCE_LEAD_LABEL = "Continue without an Account and route this person as a Lead"
PROVISION_ON_LABEL = (
...
def _projection(**changes):
    value = {
        "run_id": "run-phase4c-multi-account",
        "revision": 3,
        "workflow_key": "easyimports.list_import",
        "workflow_version": 7,
        "target_provider_id": "fake-preview-v1",
        "status": "needs_decision",
        "stage": "resolve_people_account_multi_matches",
        "decision": None,
        "effect_intent": None,
        "effect_grants": [],
        "review_handoff": None,
...
def _multi_account_decision(
    *,
    none_kind=FORCE_LEAD_KIND,
    none_label=FORCE_LEAD_LABEL,
    decision_id="decision-multi-account-phase4c",
):
    rows = [
        {
            "option_id": "opt-a1",
            ...
            "option_label": "Use this CRM account",
            "selected": True,
            "is_recommended": True,
            "candidate_account_id": "A1",
            "candidate_account_name": "Acme",
            "incoming_contact_full_name": "Jane Doe",
            "contact_email_final": "jane@incoming.example",
        ...
        },
        {
            "option_id": "opt-none",
            "group_id": "group-1",
            "option_kind": none_kind,
            "option_label": none_label,
            "selected": False,
            "is_recommended": False,
            "candidate_account_id": None,
            "candidate_account_name": None,
            "incoming_contact_full_name": "Jane Doe",
            "contact_email_final": "jane@incoming.example",
            "account_name_final": "Acme",
        },
        {
            ...
            "option_label": "Set this uploaded row aside",
            "selected": False,
            "is_recommended": False,
            "candidate_account_id": None,
            "candidate_account_name": None,
    ...
    body = {
        ...
        "decision_type": MULTI_ACCOUNT_DECISION_TYPE,
        "title": "Resolve CRM account matches",
        "message": "Choose one CRM Account, none of these, or quarantine.",
    ...
    return {
        "decision_id": decision_id,
        "decision_type": MULTI_ACCOUNT_DECISION_TYPE,
        "phase_id": "resolve_people_account_multi_matches",
        "body": body,
    }
...
def _receipt(*, outcome="rejected"):
    return {
        ...
        "resource": "/v1/workflows/run-phase4c-multi-account",
    }


class Phase4CMultiAccountFeTests(TestCase):
    def setUp(self):
        self.owner = uuid4()
    ...
    def test_strategy_and_template_characterize_multi_account(self):
        self.assertTrue(STRATEGY_PATH.is_file())
        strategy = STRATEGY_PATH.read_text(encoding="utf-8")
        self.assertIn("4C", strategy)
        self.assertIn("multi-account", strategy.lower().replace(" ", "-"))
        template = WORKFLOW_TEMPLATE.read_text(encoding="utf-8")
        self.assertIn("multiple_crm_account_matches", template)
        self.assertIn(MULTI_ACCOUNT_DECISION_TYPE, DECISION_TYPES)

    def test_renders_provision_off_lead_fallback_option(self):
        decision = _multi_account_decision()
        ...
        self.assertContains(page, FORCE_LEAD_LABEL)
        self.assertContains(page, "Use this CRM account")
        self.assertContains(page, "Set this uploaded row aside")
        ...
        self.assertEqual(
            kinds,
            {CANDIDATE_KIND, FORCE_LEAD_KIND, QUARANTINE_KIND},
        )
    ...
    def test_renders_provision_on_optional_create_option(self):
        decision = _multi_account_decision(
            none_kind=CONTINUE_NET_NEW_KIND,
            none_label=PROVISION_ON_LABEL,
        )
        value = _projection(decision=decision)
        session, workflow = self.make_session_workflow(value)
        with patch(
        ...
        self.assertContains(page, "Lead if creation is skipped")
        self.assertNotContains(page, b"must create Accounts")

    def test_none_of_these_builds_decision_command(self):
        decision = _multi_account_decision()
        value = _projection(decision=decision)
        session, workflow = self.make_session_workflow(value)
        with patch(
        ...
        self.assertEqual(validated["decision_type"], MULTI_ACCOUNT_DECISION_TYPE)
        self.assertEqual(validated["response"]["action"], ACTION)
        self.assertEqual(
            validated["response"]["groups"],
            [{"group_id": "group-1", "selected_option_id": "opt-none"}],
        )
        self.assertEqual(mutation.mutation_kind, "submit_decision")
        self.assertNotEqual(mutation.mutation_kind, "authorize_effect")


class Phase4CDjangoApiBoundaryTests(TransactionTestCase):
    """Real API process: multi-Account none-of-these resume crosses the boundary.
    ...
    def setUpClass(cls):
        ...
        for _ in range(80):
            ...
            raise RuntimeError("Phase 4C API process did not start.")
    ...
    def _start_at_multi_account(
        self,
        *,
        case_id: str,
        account_provisioning_mode: str = "disabled",
        ...
        with self.settings(
            ...
            session = ImportSession.objects.create(
                ...
                target_provider_id="fake-preview-v1",
                operator_label="Phase4C Boundary Operator",
                setup_entity="people",
            ...
            body = validate_workflow_create(
                {
                    ...
                    "maximum_modes": {
                        ...
                        "account_provisioning": account_provisioning_mode,
            ...
            self.assertEqual(
                decision["phase_id"], "resolve_people_account_multi_matches"
            )
    ...
    def _submit_none_of_these(
        ...
        *,
        ...
        self.assertEqual(
            ...
            none_option["option_id"],
        )
        self.assertEqual(mutation.response_json.get("run_id"), run_id)
        return none_option, revision_before, run_id

    def test_django_force_lead_none_of_these_accepted_by_api(self):
        with self.settings(EASYIMPORTS_API_BASE_URL=self.base_url):
            session, workflow, api = self._start_at_multi_account(
                case_id="force-lead",
                account_provisioning_mode="disabled",
            )
            page = self.client.get(reverse("importer:workflow", args=[session.id]))
            self.assertContains(page, FORCE_LEAD_LABEL)
            none_option, revision_before, run_id = self._submit_none_of_these(
                session, workflow, expected_kind=FORCE_LEAD_KIND
            )
            ...
            if refreshed.status == "needs_decision":
                self.assertNotEqual(
                    ...
                    MULTI_ACCOUNT_DECISION_TYPE,
                )
            self.assertEqual(refreshed.projection.get("effect_grants") or [], [])
            self.assertIsNone(refreshed.projection.get("effect_intent"))
            # Selected none-of-these was force_lead (provision disabled).
            self.assertEqual(none_option["option_kind"], FORCE_LEAD_KIND)

    def test_django_continue_net_new_none_of_these_accepted_by_api(self):
        with self.settings(EASYIMPORTS_API_BASE_URL=self.base_url):
            session, workflow, api = self._start_at_multi_account(
                case_id="net-new",
                account_provisioning_mode="preview",
                contacts_only=True,
            )
            page = self.client.get(reverse("importer:workflow", args=[session.id]))
            self.assertContains(page, PROVISION_ON_LABEL)
            none_option, revision_before, run_id = self._submit_none_of_these(
                session, workflow, expected_kind=CONTINUE_NET_NEW_KIND
            )
            refreshed = refresh_workflow(session, workflow, client=api)
            self.assertEqual(refreshed.run_id, run_id)
            self.assertGreater(refreshed.revision, revision_before)
            ...
            # Decision resume alone never mints write grants. Provision-configured
            # none-of-these must reach Account provisioning: non-null effect_intent
            # on the account_provisioning track (authorize surface only).
            self.assertEqual(refreshed.projection.get("effect_grants") or [], [])
            intent = refreshed.projection.get("effect_intent")
            self.assertIsNotNone(
                intent,
                "continue_net_new_account must freeze Account provisioning work "
                "and project a non-null account_provisioning effect_intent.",
            )
            self.assertEqual(intent.get("track"), "account_provisioning")
            self.assertIn(
                intent.get("maximum_mode"), {"preview", "dry_run", "execute"}
            )
            self.assertEqual(
                intent.get("effect_phase_id"), "list_account_provisioning_effect"
            )
            self.assertEqual(
                intent.get("gate_phase_id"), "freeze_list_account_provisioning_plan"
            )
            self.assertTrue(intent.get("work_digest"))
            self.assertEqual(none_option["option_kind"], CONTINUE_NET_NEW_KIND)
            self.assertIn("Lead if creation is skipped", none_option["option_label"])
